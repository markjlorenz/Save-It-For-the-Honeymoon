...
module Barby
  ...
  class DbdRmagickOutputter < Outputter

    register :to_png, :to_gif, :to_jpg, :to_image

    attr_accessor :height, :xdim, :ydim, :margin, :font_height, :background_color
    ...
    def to_image(opts={})
      with_options opts do
        canvas = Magick::Image.new(full_width, full_height+font_height/1.4){self.background_color = 'transparent'}
        ...
        result = canvas.composite(value_to_image(opts), Magick::SouthGravity, 0, 0, Magick::OverCompositeOp)
      end
    end

    def value_to_image(opts={})
      with_options opts do
        canvas = Magick::Image.new(full_width, full_height+font_height/1.4){self.background_color = 'transparent'}
        gc = Magick::Draw.new
        gc.font = "#{Rails.root}/public/fonts/StRydeRegular.ttf"
        gc.text(0, 0, barcode.data)
        gc.fill('black')
        gc.pointsize = font_height
